...
cat(",p=",p)
ratio_hhgl = as.numeric(args[9]) # vaccination coverage
cat(",ratio_hhgl=",ratio_hhgl)
...
for (i in 1:nSim){
  ...
  epi.outbreak[[i]]<-generation.interval.parallel.vacc(R=R,n=n,mu=mu,tv=tv,p=p)
  }
...
name<-paste("CompScenario_","n",n,"_R",R,"_mu",mu, "_IPDist",IPDIST, "_Rate",RATE, "NinF",n.InInf ,"_.RData", sep = "")
setwd(out)
save(MeanGT, file = name)
